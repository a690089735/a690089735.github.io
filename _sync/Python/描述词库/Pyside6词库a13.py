...
for root, dirs, files in os.walk(os.path.dirname(sys.argv[0])):
    dataPaths = natsort.os_sorted(
        [os.path.join(root, file) for file in files if file[-5:].lower() == ".json"]
    )
    break
...
class WordLibrary(QTabWidget):
    ...
    selectionChange = Signal(int, str)
            ...
            for header, parameters in content.items():
                ...
                for parameter, value in parameters.items():
                    ...
                    qPushButton.clicked.connect(
                        (
                            lambda qPushButton=qPushButton, textcn=parameter, texten=value[
                                "text"
                            ], order=value[
                                "order"
                            ]: self.buttonClicked(
                                qPushButton, textcn, texten, order
                            )
                        )
    ...
    def buttonClicked(self, sender: QPushButton, textcn: str, texten: str, order: int):
        ...
        if self.selectionCount > 0:
            tabName = f"{self.dataName} {self.selectionCount}"
        else:
            tabName = self.dataName
        self.selectionChange.emit(self.index, tabName)
