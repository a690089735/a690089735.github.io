...
# 本版Alpha2.0
# 要求添加识别介词的功能,经过设计,添加到json属性会在后期传递时加大处理难度,词条末尾为"#&"时,不对词条添加逗号.(实际是删除了匹配的"#&,*")
# 由json数据改为Excel表格数据,方便维护.支持(".xls", ".xlsb", ".xlsm", ".xlsx")四种格式,其实其他格式也能支持,但是没安相关环境,未做测试(临时文件冲突和文件读取),故不提供.
...
def chars2Codes(text: str):
    for char, code in ccPairs:
        text = text.replace(char, code)
    print(text)
    return text


def codes2Chars(text: str):
    for char, code in ccPairs:
...
dataPaths = []
for root, dirs, files in os.walk(os.path.join(os.path.dirname(sys.argv[0]), "data")):
...
def combineText(strList: list, showName: bool = False, color: bool = False, char2code=False):
    result = ""
    tid = 1
    if showName:
        tid = 0
    ...
    for strItems in strList:
        ...
        for str in strItems:
            ...
            result += f"{chars2Codes(cstr) if char2code else cstr},"
        if color:
            result += "</span>"
    result = removeLinkComma.sub("", removeLastComma.sub("</span>", normalizeComma.sub(r", ", result)))  # 果然还是正则优雅啊...顺带着还优化了一大堆格式问题.
    return result
...
class WordLibrary(QTabWidget):
    ...
    def buttonClicked(
        ...
        self.textChange.emit(combineText(parameters, showName=showName, color=True, char2code=True))
        # tabName = ""
...
class ControlBoard(QWidget):
    ...
    def language(self):
        ...
        if showName:
            ...
            sender.setText("英>中")
        self.showNameChange.emit(combineText(parameters, showName=showName, color=True, char2code=True))

    def copyText(self):
        pyperclip.copy(combineText(parameters))
...
if __name__ == "__main__":
    ...
    class Window(QWidget):
        def __init__(self):
            ...
            tabs = QTabWidget()
            i = -1
            for data, value in dictData.items():
                i += 1
            ...
            self.setWindowTitle("描述词库2.0.1 Alpha")
