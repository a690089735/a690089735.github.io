...
class Siki(object):
    def __init__(self) -> None:
        ...
        self.max_worker = 80
        # self.page_url = "http://www.sikiedu.com/my/courses/learning?page=" ##全部课程
        self.page_url = "https://www.sikiedu.com/my/courses/favorite?page=" ##收藏课程
        # 班级 https://www.sikiedu.com/classroom/5/courses
        self.page_index = "1"
    ...
    async def get_learning_list(self):
        self.page_index = input("输入页码(请自行到https://www.sikiedu.com/my/courses/favorite获取):")
    ...
    async def get_course_detail_list(self, courseid, coursename):
        courseid = str(int(courseid) + 3)
        url = f"http://www.sikiedu.com/my/course/{courseid}"
        response = await self.session.get(url, headers={"Referer": ("https://www.sikiedu.com/classroom/5/courses"), "user-agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.137 Safari/4E423F"})
        html = etree.HTML(await response.text())
    ...
    async def download_task(self, *, courseid, taskid, name, coursename, **kwargs):
        ...
        # 第一层
        logger.debug("解析第一层")
        print(f'http://www.sikiedu.com/course/{courseid}/task/{taskid}/show')
        activity_show_response = await self.session.get(f"http://www.sikiedu.com/course/{courseid}/task/{taskid}/show")
        logger.debug("获取1")
        temp = await activity_show_response.text()
        print(f'-{temp}-')
        HTML = etree.HTML(temp)
        # print(HTML)
        logger.debug("获取2")
        info_div = HTML.xpath("//div[(@data-file-global-id)]")[0]
        logger.debug(info_div)
        logger.debug("获取3")
        # file_global_id = info_div.xpath("./@data-file-global-id")[0]
        data_url = "https://www.sikiedu.com/hls/21791/playlist/Bb8GpyWoH3cptDca794FkSlT6ApL2Crs.m3u8"
